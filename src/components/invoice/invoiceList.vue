<template>
  <div class="invoice-wrapper px-2">
    <div class="d-mobile">
      <v-card
        class="my-4 px-5 pb-2 pt-5 secondary rounded-lg invoice-card"
        v-for="invoice in filter"
        :key="invoice.id"
        @click="newPage(invoice.id)"
      >
        ...
        <div class="d-flex justify-space-between">
          <div>
            ...
            <p class="font-weight-bold price">
              &#xa3;{{
                (invoice.total = formatCurrency(getGrandTotal(invoice.items)))
              }}
            </p>
          </div>
          <v-btn
            class="text-capitalize lighten-5 py-5"
    ...
    <div class="d-desktop">
      <v-card
        class="my-4 px-5 pa-2 py-4 secondary rounded-lg invoice-card card-desktop main-flex"
        v-for="invoice in filter"
        :key="invoice.index"
        @click="newPage(invoice.id)"
      >
        ...
        <div class="right flex-item">
          ...
          <v-btn
            class="text-capitalize rounded pa-0 px-7 py-5"
            :class="`status ${invoice.status}`"
            text
            ><span :class="`mr-3 dot ${invoice.status}`"></span>
            {{ invoice.status }}</v-btn
